...
public class AggregatorSum {
	...
	public static void main(String[] args) throws Exception {
        ...
        props.put(StreamsConfig.APPLICATION_ID_CONFIG, "streams-aggregator-sum");
        props.put(StreamsConfig.CLIENT_ID_CONFIG, "streams-aggregator-sum-client");
        ...
        try {
        	KStream<String, Double> sum_data = builder.stream(sum_topic, Consumed.with(Serdes.String(), Serdes.Double()));
        	KStream<String, String> sum_aggr_stream = sum_data
            		.groupByKey(Grouped.with(Serdes.String(), Serdes.Double()))
    				.windowedBy(TimeWindows.of(Duration.ofSeconds(window)))
    				.reduce((v1,v2) -> v1 + v2 )
    				.toStream()
    				.map((key,value) -> new KeyValue<String,String>(key.toString(),getLineProtocol(key,value,"sum"))); 
        	sum_aggr_stream.to(results_topic);
        } catch (Exception e) {
	    	e.printStackTrace();
	    }
        ...
        Runtime.getRuntime().addShutdownHook(new Thread("aggregator-sum-shutdown-hook") {
            @Override
            public void run() {
                streams.close();
                latch.countDown();
